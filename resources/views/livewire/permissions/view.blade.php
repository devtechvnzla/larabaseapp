@section('title', __('Users'))
<div>
    <div class="row g-4 mb-4">
        <div class="col-sm-6 col-xl-4">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-start justify-content-between">
                        <div class="content-left">
                            <span>Permisos</span>
                            <div class="d-flex align-items-end mt-2">
                                <h3 class="mb-0 me-2">{{ \App\Models\Permission::count() }}</h3>
                                <small class="text-success">(100%)</small>
                            </div>
                            <small>Total Registrados</small>
                        </div>
                        <span class="badge bg-label-primary rounded p-2">
                            <i class="fa fa-user ti-sm"></i>
                        </span>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 col-xl-4">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-start justify-content-between">
                        <div class="content-left">
                            <span>Permisos inactivos</span>
                            <div class="d-flex align-items-end mt-2">
                                <h3 class="mb-0 me-2">{{ \App\Models\Permission::where('status',0)->count() }}</h3>
                                <small class="text-danger">(+95%)</small>
                            </div>
                            <small>Total en espera </small>
                        </div>
                        <span class="badge bg-label-danger rounded p-2">
                            <i class="fas fa-user-check ti-sm"></i>
                        </span>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 col-xl-4">
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-start justify-content-between">
                        <div class="content-left">
                            <span>Permisos activos</span>
                            <div class="d-flex align-items-end mt-2">
                                <h3 class="mb-0 me-2">{{ \App\Models\Permission::where('status',1)->count() }}</h3>
                                <small class="text-success">(+95%)</small>
                            </div>
                            <small>Total de activos</small>
                        </div>
                        <span class="badge bg-label-success rounded p-2">
                            <i class="fas fa-user-check ti-sm"></i>
                        </span>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                <h4 class="card-title">
                    Listado de permisos
                </h4>
                   <div class="card-controls">
                     <div class="row">
                         <div class="my-1 col-sm-2">

                        <select wire:model="perPage" class="form-select w-full sm:w-1/6">
                            @foreach($paginationOptions as $value)
                                <option value="{{ $value }}">{{ $value }}</option>
                            @endforeach
                        </select>
                  </div>
                  <div class="my-1 col-sm-7">
                       @can('permission_create')
                        <div class="btn  btn-info" data-bs-toggle="modal" data-bs-target="#createDataModal">
                           <i class="fa fa-plus"></i>  Nuevo permiso
                         </div>
                       @endcan
                    @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))

                    <x-excel-export/>
                    @endif


                </div>
                <div class="my-1 col-sm-3">
                     <input type="text" wire:model.debounce.300ms="search" class="form-control" placeholder="Búsqueda de datos" />
                </div>
                   </div>
                  </div>
              </div>

                <div class="card-body">
						@include('livewire.permissions.create')
						@include('livewire.permissions.update')
				<div class="table-responsive">
					<table class="table table-bordered table-sm">
						<thead class="thead">
							<tr class="text-center">

								<td>#
                                    @include('components.table.sort', ['field' => 'id'])
                                </td>
								<th>Título
                                    @include('components.table.sort', ['field' => 'name'])
                                </th>
								<th>Descripción
                                    @include('components.table.sort', ['field' => 'descripcion'])
                                </th>
								<th>Módulo
                                    @include('components.table.sort', ['field' => 'slug'])
                                </th>
                                <th>Estado del permiso
                                    @include('components.table.sort', ['field' => 'status'])
                                </th>
								<td>ACTIONS</td>
							</tr>
						</thead>
						<tbody>
							@foreach($permissions as $row)
							<tr class="text-center">

								<td>{{ $loop->iteration }}</td>
								<td>{{ $row->name }}</td>
								<td>{{ $row->descripcion }}</td>
								<td>{{ $row->slug }}</td>
                                <td>

                                        @if ($row->status == 1)
                                            <span class="badge bg-label-success">
                                                ACTIVO
                                            </span>

                                         @else
                                            <span class="badge bg-label-danger">
                                                INACTIVO
                                            </span>
                                        @endif


                                </td>
								<td width="150">
								<div class="btn-group">

									@can('permission_edit')
                                    <a data-bs-toggle="modal" data-bs-target="#updateModal" class="text-white btn btn-primary" wire:click="edit({{$row->id}})"><i class="fa fa-edit"></i>  </a>
                                    @endcan
                                    @can('permission_delete')
                                    <a class="text-white btn btn-danger" wire:click.prevent="deleteConfirmation({{$row->id}})"><i class="fa fa-trash"></i>  </a>
                                    @endcan

								</div>
								</td>
                            </tr>
							@endforeach
						</tbody>
					</table>
					</div>
				</div>

			<div class="card-footer clearfix">
                      {{ $permissions->links() }}
                      <p class="text-muted">Mostrando <strong>{{ $permissions->count() }}</strong> registros de <strong>{{$permissions->total() }}</strong> totales</p>
                  </div>
            </div>
		</div>
	</div>
</div>
@push('scripts')
<script>
    window.addEventListener('show-delete-confirmation', event =>{
        Swal.fire({
          title: '¿Estás seguro(a)?',
          text: "¡No podrás revertir esto!",
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          confirmButtonText: 'Yes, delete it!'
        }).then((result) => {
          if (result.isConfirmed) {
            Livewire.emit('deleteConfirmed');
          }
    })
    });

</script>
@endpush
